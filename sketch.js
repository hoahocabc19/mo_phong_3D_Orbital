// sketch.js — Updated: silence font load log and hide Ready status
// - Removed console.log from font load callback so no "Font loaded from ..." appears.
// - Do not show "Ready" in the top-right status; statusDiv is only shown for warnings/errors.
// - Progress DOM (#progress) still used during sampling.
// - All other behavior unchanged except: disable manual rotation when left-clicking on UI elements.
...
function mousePressed(event) {
  ...
  if (!_eventIsOnCanvas(event)) {
    // optionally, if you want clicks on labels/buttons to cancel any ongoing dragging:
    // isDragging = false;
    return;
  }
